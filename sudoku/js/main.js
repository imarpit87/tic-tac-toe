...
let selectedAvatar=null; avatarPicker?.addEventListener('click', (e)=>{ const b=e.target.closest('button.avatar, .avatar'); if(!b) return; avatarPicker.querySelectorAll('.avatar').forEach(x=>x.classList.remove('selected')); b.classList.add('selected'); selectedAvatar=b.dataset.avatar||null; });
